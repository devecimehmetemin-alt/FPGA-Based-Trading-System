...
Outputs into --outdir (suppress with --no-vectors):
  ...
  mold_beats.hex    one 64-bit AXI-Stream beat per line, for $readmemh
...
A mold_beats.hex line is 20 hex digits holding {6'b0, 1'b0, last, keep[7:0],
data[63:0]} -- read it into a logic [79:0] array. Bytes pack little-endian
within a beat, so packet byte 0 lands in data[7:0], and last is set on the
final beat of every packet. Bit 73 is where eth_beats.hex carries fcs_ok; it is
always zero here, so compare bits [72:0]. This is byte for byte what
